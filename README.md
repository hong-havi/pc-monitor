...
2. 브라우저에서 다운로드 차단 경고가 뜨면 `⋯` → **유지** → **이 파일을 안전한 것으로 보고** 클릭
3. `PCMonitor.exe`를 **우클릭** → **관리자 권한으로 실행**
4. Windows SmartScreen 경고가 뜨면 **추가 정보** → **실행** 클릭

> ⚠️ 코드 서명이 없는 개인 프로젝트라 브라우저/Windows에서 경고가 표시됩니다. 소스코드가 공개되어 있으니 안심하고 사용하셔도 됩니다.
